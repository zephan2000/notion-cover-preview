...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      if (error || !data) {
        return res.status(404).json({ error: 'Config not found or expired', detail: error?.message });
      }
    ...
    if (req.method === 'PATCH') {
      ...
      if (error) {
        return res.status(500).json({ error: 'Failed to save selections', detail: error.message });
      }
